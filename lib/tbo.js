...
export class TBO {
    ...
    constructor(gal, layout = this.constructor.layout) {
        this.gal     = gal;
        this.layout  = layout;
        this.texture = gal.device.createTexture({ label: `tbo:${id++}`, array: true, ...layout });
    }
    ...
    upload() {
        const { bytesPerRow, texture, width, height, layers } = this;
    ...
    static Layout = class TBOLayout {
        constructor({ format = 'rgba32float', width, height, limit }) {
            ...
            this.usage  = TEXTURE_USAGE.TEXTURE_BINDING | TEXTURE_USAGE.COPY_DST;
        }
