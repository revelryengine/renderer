...
class FreeList {
    ...
    insertRegion(region) {
        ...
        do {
            if(search.offset < region.offset && search.next.offset > region.offset) {
                region.prev = search;
    ...
    splitRegion(region, bytes) {
        ...
        if(region === this.tail) this.tail = split;
    ...
    #mergeRegion(region) {
        ...
        if(prev !== this.tail && ((prev.offset + prev.bytes) === region.offset)) {
        ...
        if(next !== this.head && ((region.offset + region.bytes) === next.offset))  {
...
export class TBO {
    ...
    constructor(gal, layout = this.constructor.layout, double = this.constructor.double) {
        ...
        this.#nextFitStart = this.#free.head;
    }
    ...
    alloc(n) {
        ...
        let region = this.#free.search(bytes, this.#nextFitStart) ?? this.#free.grow();

        if(region.bytes > bytes) {
            region = this.#free.splitRegion(region, bytes);
        }

        this.#nextFitStart = region.next;
...
export class MatrixTBO extends TBO {
    createMatrixViewBlock(n) {
        ...
        for(let i = 0; i < n; i++) {
            ...
            block.push({ matrixView: view });
        }
        return { index: region.offset / 64, block };
    }
