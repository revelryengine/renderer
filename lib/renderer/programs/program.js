...
function createBlockSetter(context, location, program) {
    return (binding) => {
        context.uniformBlockBinding(program, location, binding);
    }
}

function createSamplerSetter(context, unit, target = GL.TEXTURE_2D) {
    return (texture) => {
...
class BlockSetter extends ShaderSetter {
    constructor(context, program) {
        ...
        for (let i = 0; i < blockCount; i++) {
            ...
            this.locations[name] = i;
            this.setters[name]   = createBlockSetter(gl, i, program);
        }
    }
}

/**
* A sampler setter is used to set samplers by name
*/
    ...
    */
    ...
    async compile({ sync = false } = {}, abortCtl){
        ...
        if(!compiled) {
            ...
            const blocks     = new BlockSetter(gl, program);
            ...
            for(const [name, location] of Object.entries(this.constructor.uniformBindings)){
