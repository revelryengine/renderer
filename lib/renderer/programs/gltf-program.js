...
export class GLTFProgram extends Program {
  ...
  constructor(context, primitive, node, graph, additionalDefines = {}) {
    ...
    const { ibl, punctual, ssao, shadows, debug } = graph.settings;
    ...
    const shadowMultiplier = { [LIGHT_TYPES.spot]: 1, [LIGHT_TYPES.directional]: 0 };
    ...
    const defines = {
      ...
      USE_SHADOWS:  shadows.enabled && shadowCount ? 1 : null,
  ...
  run(primitive, node, graph, input) {
    ...
    this.applyMaterial(context, material, graph, input);  
    ...
    this.uniforms.set('u_ShadowSamplers', graph.shadows?.glTexture);
    this.uniforms.set('u_ShadowMatrices', graph.shadows?.matrices);
  ...
  applyMaterial(context, material, graph, input) {
    const {
      pbrMetallicRoughness = {},
    ...
    for(const name in material?.extensions) {
      material.extensions[name]?.applyMaterial?.(this, context, material, graph, input);
    }
  }
