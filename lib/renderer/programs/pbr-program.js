...
export class PBRProgram extends Program {
  ...
  run(primitive, node, graph) {
    ...
    const { cameraPosition, viewMatrix, projectionMatrix, viewProjectionMatrix, lights, environment } = graph;
    ...
    this.uniforms.set('u_SSAOSampler', graph.passes.ssao?.blurTexture);
    this.uniforms.set('u_ViewportDimensions', [graph.viewport.width, graph.viewport.height]);
