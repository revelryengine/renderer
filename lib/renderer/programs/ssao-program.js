...
export class SSAOProgram extends Program {
    ...
    run(graph, input, output) {
        ...
        const { depth, normal, radius, bias } = input;

        const { projectionMatrix, invProjectionMatrix } = graph.viewInfo;

        const noiseScale = [output.ssao.width / 4, output.ssao.height / 4]; //output.ssao.width;

        this.uniforms.set('u_NoiseSampler', noiseTexture);
        this.uniforms.set('u_DepthSampler', depth.glTexture);
        this.uniforms.set('u_NormalSampler', normal.glTexture);
