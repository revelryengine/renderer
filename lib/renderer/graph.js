...
export class Graph {
  ...
  /**
   * Returns a sorted list of primitives with their nodes. This allows for rendering alpha blend modes in the correct
   * order.
  ...
  getSortedPrimitives(viewMatrix) {
    ...
    for(const { primitive, node } of this.primitives) {
      ...
      // if(pos[2] < 0) // I keep coming back to this but it just doesn't make sense to cull based on depth of centroid (i.e the plane of a primitive may enter the viewport without the centroid)
      sorted.push({ depth: -pos[2], primitive, node, blend });
    }

    return sorted.sort((a, b) => a.blend - b.blend || b.depth - a.depth);
  }
}
