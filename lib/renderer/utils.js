...
export function createBuffer(context, {
  ...
  componentType = GL.BYTE, byteOffset = 0, byteLength,
}) {
  const buffer = context.createBuffer();
  const array = new componentTypeMap[componentType](data.slice(byteOffset, byteOffset + byteLength));
...
function createAttribSetter(context, location) {
  return (
    ...
    normalize = false, stride = 0, offset = 0, target = context.ARRAY_BUFFER,
  ) => {
    context.bindBuffer(target, buffer);
