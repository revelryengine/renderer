...
const defaultParams = {
    color: {
        min: GL.LINEAR, mag: GL.NEAREST, wrapS: GL.CLAMP_TO_EDGE, wrapT: GL.CLAMP_TO_EDGE,
...
export class FBO {
    ...
    createFramebuffer() { 
        ...
        if(attachments.depth)   this.attachTexture('depth',   attachments.depth,   gl.DEPTH_ATTACHMENT);
        if(attachments.stencil) this.attachTexture('stencil', attachments.stencil, gl.DEPTH_STENCIL_ATTACHMENT);
    ...
    setDrawbuffers(buffers) {
        ...
        gl.bindFramebuffer(gl.FRAMEBUFFER, this.framebuffer);
        gl.drawBuffers(buffers || this.attachments.colors.map((_, i) => gl.COLOR_ATTACHMENT0 + i));
        gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    }
    ...
    createRenderbuffers(samples) {
        ...
        if(attachments.depth)   this.attachRenderbuffer('depth',   attachments.depth,   gl.DEPTH_ATTACHMENT, samples);
        if(attachments.stencil) this.attachRenderbuffer('stencil', attachments.stencil, gl.DEPTH_STENCIL_ATTACHMENT, samples);
...
//         for(let i = 0; i < attachments.colors.length; i++) {
//             this.attachRenderbuffer('color', attachments.colors[i], gl.COLOR_ATTACHMENT0 + i, maxSamples);
//         }
//         if(attachments.depth)   this.attachRenderbuffer('depth',   attachments.depth,   gl.DEPTH_ATTACHMENT, maxSamples);
//         if(attachments.stencil) this.attachRenderbuffer('stencil', attachments.stencil, gl.DEPTH_STENCIL_ATTACHMENT, maxSamples);

        
//         const status = gl.checkFramebufferStatus(gl.FRAMEBUFFER);
//         if(status != gl.FRAMEBUFFER_COMPLETE){
//             console.warn('Renderbuffer error:', this.fbo.name, FBO.GL_FRAMEUBUFFER_STATUS_ERRORS[status]);
//         }
...
//     attachRenderbuffer(textureType, texture, attachment, samples) {
//         const { context: gl, framebuffer, width, height } = this.fbo;
