...
/**
 * @todo: use KHR_parallel_shader_compile and make program compilation async
 */
...
export class Renderer {
  ...
  destroy() {
    this.glExtensions.WEBGL_lose_context.loseContext();
  }
}

export class RendererXR extends Renderer {
  constructor({ ibl } = {}){
    const canvas  = document.createElement('canvas');
    const context = canvas.getContext('webgl2', { xrCompatible: true });
    super(context, { ibl, autoresize: false });
  }

  render(scene, xrRefSpace, xrFrame) {
    const { context: gl } = this;
    ...
    if(pose) {
      ...
      for (const view of pose.views) {
        ...
        const cameraPosition = vec3.create();
        const cameraTransform = pose.transform.matrix;//this.graph.getWorldTransform(pose.transform.matrix);
