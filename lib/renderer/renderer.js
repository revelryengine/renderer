...
export class Renderer {
  ...
  constructor(target, { settings, xrCompatible = false } = {}) {
    this.settings = Object.assign({
      ...
      dof:      { enabled: true, distance: 5, range: 5     },
      debug:    'DEBUG_NONE',
    }, settings);
    ...
    this.pipeline = new StandardPipeline(this.context);
  }
  ...
  render(scene, cameraNode, output = this.context.canvas) {
    ...
    const { context: gl, pipeline, graph, environment, defaultLights } = this;
    ...
    graph.analyze({ scene, cameraNode, viewport: { width, height }, environment });

    if(!graph.lights.length) {
      graph.lights.push(...defaultLights);
    }

    pipeline.run(graph, output);
  }
  ...
  getDistanceAtPoint(x, y) {
    ...
    gl.readPixels(x / xRatio, y / yRatio, 1, 1, gl.RED, gl.FLOAT, output);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);

    const depth = output[0];
    const distance = near * far / (far + depth * (near - far));
    return distance;
  }
  ...
  getPrimitiveAtPoint(x, y) {
    ...
    const id = output[0];
    for(const { primitive } of this.graph.primitives) {
      if(this.graph.getPrimitiveId(primitive) === id) {
        return primitive;
      }
    }
  }
