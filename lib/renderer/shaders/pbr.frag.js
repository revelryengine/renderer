 ...
 * 
 * Modifications:
 * - @modification-pre-pass Added support for outputing position and normals to textures
...
uniform sampler2DShadow u_ShadowSamplers[SHADOW_COUNT];
uniform mat4 u_ShadowMatrices[SHADOW_COUNT];
in vec4 v_ShadowTexcoords[SHADOW_COUNT];
#endif
/************  /@modification-shadows [1/2]****************/
...
/************  /@modification-ssao [2/2] ****************/
    ...
    for (int i = 0; i < LIGHT_COUNT; ++i)
    {
        ...
        if (NdotL > 0.0 || NdotV > 0.0)
        {
            ...
            #ifdef USE_SHADOWS
                vec4 texcoord = v_ShadowTexcoords[i];
                texcoord.z -= 0.0001 * tan(acos(NdotL)); // Apply slope-scale bias
                float shadow = textureProj(u_ShadowSamplers[0], texcoord);
            #else
                float shadow = 1.0;
            #endif
