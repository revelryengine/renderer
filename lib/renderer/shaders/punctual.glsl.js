...
uniform float u_ShadowSplits[SHADOW_CASCADES + 1];
...
float shadowPCF(int shadowLayer, float bias) 
{
    ...
    texcoord.z -= bias;// Apply slight offset to reduce artifacts
...
vec2 getShadowCascade() 
{
    ...
    float depth = linearDepth(gl_FragCoord.z * 0.5 + 0.5, u_ShadowSplits[0], u_ShadowSplits[SHADOW_CASCADES]);
...
/**
 * offset = (float)(frac(position.xy * 0.5) > 0.25);  
 * // mod offset.y += offset.x;  
 * // y ^= x in floating point 
  ...
  shadowCoeff = (offset_lookup(shadowmap, sCoord, offset + float2(-1.5, 0.5)) 
              ...
              + offset_lookup(shadowmap, sCoord, offset + float2(0.5, -1.5)) ) * 0.25; 
 */
