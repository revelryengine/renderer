...
void main(void) {
    ...
    vec2 offsetScale = vec2(0.0, 1.0) * texelSize;
        ...
        vec3 centerColor = texture(u_InputSampler, v_TexCoord).rgb;
        vec3 result = centerColor * 2.0 * gaussianCoeff[0];
        float normalization = 2.0 * gaussianCoeff[0];
        for (int i = 1; i < sampleCount; i++) {
            vec2 offset = colorSamples[i] * offsetScale;

            vec3 a = texture(u_InputSampler, v_TexCoord + offset).rgb;
            vec3 b = texture(u_InputSampler, v_TexCoord - offset).rgb;

            float aCloseness = 1.0 - distance(a, centerColor) / length(vec3(1.0));
            float bCloseness = 1.0 - distance(b, centerColor) / length(vec3(1.0));
        ...
        g_finalColor = vec4(bilateral, 1.0);
    #else
