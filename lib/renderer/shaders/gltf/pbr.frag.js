 ...
 * 
 * Modifications:
 * - @modification-base Added support for outputing depth and normal to a texture
 * - @modification-debug-normal-view Added support for outputing view space normal
 * - @modification-ssao Added support for SSAO by calculating AO from depth map genreated during transmission opaque framebuffer pass
 * - @modification-shadows Added support for shadow maps
 * - @modification-debug-shadow-cascade Added support for outputting frustum cascades
 * - @modification-fog Added support for depth fog
 ...
 * - @modification-environment-ubo Added support for using uniform buffers for environment
 ...
 * - @modification-bloom Added support for bloom effect
 */

import { tonemapping    } from './tonemapping.glsl.js';
import { textures       } from './textures.glsl.js';
import { functions      } from './functions.glsl.js';
import { brdf           } from './brdf.glsl.js';
import { punctual       } from './punctual.glsl.js';
import { ibl            } from './ibl.glsl.js';
import { materialInfo   } from './material-info.glsl.js';
import { frustum        } from '../frustum.glsl.js';
import { environment    } from '../environment.glsl.js';
import { lighting       } from '../lighting.glsl.js';
import { material       } from '../material.glsl.js';
...
/************  @modification-lighting-ubo [1/2]****************/
${lighting}
/************  /@modification-lighting-ubo [1/2]****************/

/************  @modification-material-ubo [1/1]****************/
${material}
/************  /@modification-material-ubo [1/1]****************/
...
//find replace u_ModelMatrix -> v_ModelMatrix below
/************  /@modification-instance [1/1]****************/

/************  @modification-base [1/2]****************/
//out vec4 g_finalColor;
layout(location = 0) out vec4 g_finalColor;

#ifdef BASE_PASS
    layout(location = 1) out vec4 g_point; //depth and normal
    layout(location = 2) out uvec2 g_ids;
    flat in uvec2 v_PrimitiveId;
#endif
/************  /@modification-base [1/2]****************/

/************  @modification-ssao [1/2]****************/
#ifdef USE_SSAO
...
void main()
{
    ...
    vec3 v = normalize(u_FrustumPosition - v_Position);
    /************  /@modification-frustum-ubo [1/1]****************/
...
/************  @modification-base [2/2]****************/

#ifdef BASE_PASS
    vec3 viewN = normalize(mat3(u_InvViewMatrix) * n);
    g_point = vec4(gl_FragCoord.z, (viewN * 0.5 + 0.5));
    g_ids   = v_PrimitiveId;
#endif
...
/************  /@modification-base [2/2]****************/
    ...
    f_transmission += materialInfo.transmissionFactor * getIBLVolumeRefraction(
        ...
        v_Position, v_ModelMatrix, u_ViewMatrix, u_ProjectionMatrix,
        materialInfo.ior, materialInfo.thickness, materialInfo.attenuationColor, materialInfo.attenuationDistance);
#endif

    float ao = 1.0;
    
/************  @modification-ssao [2/2] ****************/
#ifdef USE_SSAO
#ifndef HAS_OCCLUSION_MAP
    vec2 uv = gl_FragCoord.xy / vec2(u_ViewportWidth, u_ViewportHeight);
    ao = texture(u_SSAOSampler, uv).r;
    f_diffuse = mix(f_diffuse, f_diffuse * ao, 1.0);
    f_specular = mix(f_specular, f_specular * ao, 1.0);
    f_sheen = mix(f_sheen, f_sheen * ao, 1.0);
    f_clearcoat = mix(f_clearcoat, f_clearcoat * ao, 1.0);
#endif
#endif
/************  /@modification-ssao [2/2] ****************/
    ...
    /************  /@modification-lighting-ubo [2/2]****************/
    {
        ...
        if (NdotL > 0.0 || NdotV > 0.0)
        {
            ...
            f_diffuse += intensity * NdotL *  BRDF_lambertian(materialInfo.f0, materialInfo.f90, materialInfo.c_diff, materialInfo.specularWeight, VdotH) * shadow;
            f_specular += intensity * NdotL * BRDF_specularGGX(materialInfo.f0, materialInfo.f90, materialInfo.alphaRoughness, materialInfo.specularWeight, VdotH, NdotL, NdotV, NdotH) * shadow;
            /************  /@modification-shadows ****************/
        ...
        vec3 transmissionRay = getVolumeTransmissionRay(n, v, materialInfo.thickness, materialInfo.ior, v_ModelMatrix);
...
/************  @modification-fog [1/1]****************/
#ifdef USE_FOG
    float dist = getLinearDepth(gl_FragCoord.z * 0.5 + 0.5);
    float fog = (u_FogRange.y - dist) / (u_FogRange.x - u_FogRange.y);
    fog = clamp(fog, 0.0, 1.0);
    g_finalColor = mix(g_finalColor, u_FogColor, fog);
#endif
/************  /@modification-fog [1/1]****************/
...
/************ @modification-debug-normal-view ****************/
#if DEBUG == DEBUG_NORMAL_VIEW
    vec3 debugN = normalize(mat3(u_InvViewMatrix) * n);
    g_finalColor.rgb = (debugN * 0.5) + 0.5;
#endif
/************ /@modification-debug-normal-view ****************/
...
/************ @modification-debug-shadow-cascade ****************/
#if DEBUG == DEBUG_SHADOW_CASCADE
    g_finalColor = vec4(toneMap(color), baseColor.a);
#ifdef USE_SHADOWS
    vec2 cascade = getShadowCascade();

    if(cascade.x == 0.0) {
        g_finalColor.rgb *= vec3(1.0, 0.0, 0.0);
    } else if(cascade.x == 1.0) {
        g_finalColor.rgb *= vec3(0.0, 1.0, 0.0);
    } else if(cascade.x == 2.0) {
        g_finalColor.rgb *= vec3(0.0, 0.0, 1.0);
    } else if(cascade.x == 3.0) {
        g_finalColor.rgb *= vec3(1.0, 1.0, 0.0);
    } else if(cascade.x == 4.0) {
        g_finalColor.rgb *= vec3(1.0, 0.0, 1.0);
    } else if(cascade.x == 5.0) {
        g_finalColor.rgb *= vec3(0.0, 1.0, 1.0);
    }
#endif
#endif
/************ /@modification-debug-shadow-cascade ****************/
