...
function setAttribute(attribute, accessor, contextMap) {
  const { bufferView: { byteStride }, componentType, normalized, numberOfComponents, byteOffset } = accessor;
...
export class PBRProgram extends Program {
  ...
  constructor({ context, primitive }) {
    ...
    const { NORMAL, TANGENT, TEXCOORD_0, COLOR_0, JOINTS_0 } = primitive.attributes;
    const define = {
  ...
  run(primitive, cameraTranslate, modelMatrix, mvpMatrix, contextMap, weights, joints) {
    this.use();
    ...
    if (joints) this.uniforms['u_JointMatrix[0]'].set(joints);
    ...
    const { POSITION, NORMAL, TANGENT, TEXCOORD_0, COLOR_0, JOINTS_0, WEIGHTS_0 } = primitive.attributes;
    ...
    if (this.define['HAS_SKIN']) {
      setAttribute(this.attributes['a_Joints0'], JOINTS_0, contextMap);
      setAttribute(this.attributes['a_Weights0'], WEIGHTS_0, contextMap);
    }
