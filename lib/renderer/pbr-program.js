...
export class PBRProgram extends Program {
  ...
  run(
    ...
    for(let i = 0; i < targets.length; i++) {
      for(const name in targets[i]) {
        const attr = targets[i][name];
        this.attributes.set(`a_Target_${i}_${name}`, attr);
      }
    }

    this.setTextureUniformsForObject(context, pbrMetallicRoughness);
    this.setTextureUniformsForObject(context, material);
    ...
    if(material.extensions) {
      for(const name in material.extensions) {
        const ext = material.extensions[name]
  ...
  setTextureUniformsForObject(context, object) {
    for(const name in object){
      const texture = object[name];
