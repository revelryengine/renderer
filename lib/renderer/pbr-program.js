...
function setAttribute(attribute, accessor, contextMap) {
  const { bufferView: { componentType, byteStride }, normalized, type, byteOffset } = accessor;
  attribute.set(contextMap.get(accessor.bufferView), sizeMap[type], componentType, normalized, byteStride, byteOffset);
}
...
export class PBRProgram extends Program {
  ...
  run(primitive, cameraTranslate, modelMatrix, mvpMatrix, contextMap) {
    this.use();
    ...
    const {
      pbrMetallicRoughness, normalTexture, occlusionTexture, emissiveTexture, emissiveFactor, doubleSided,
    } = primitive.material;
    ...
    gl.bindBuffer(primitive.indices.bufferView.target, contextMap.get(primitive.indices.bufferView));
    gl.drawElements(primitive.mode, primitive.indices.count, primitive.indices.componentType, null);
  }
}
