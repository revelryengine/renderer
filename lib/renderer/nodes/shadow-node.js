...
export class ShadowNode extends RenderNode {
    ...
    static output = {
        shadows: { type: 'texture', attachmentType: 'depth', 
            params: { 
                array: true, min: GL.LINEAR, mag: GL.LINEAR,
    ...
    render(graph) {
        ...
        for(const light of graph.lights) {
            ...
            gl.framebufferTextureLayer(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, this.output.shadows.glTexture, 0, layer++);
            ...
            mat4.multiply(matrix, matrix, this.shadowGraph.viewInfo.projectionMatrix);
            mat4.multiply(matrix, matrix, mat4.invert(mat4.create(), this.cameraNode.matrix));

            this.output.shadows.matrices.push(matrix); 
        }
