...
export class ShadowNode extends GeometryNode {
    opaque = true;
    ...
    constructor(pipeline) {
        ...
        this.shadowFrustum = new Frustum({ alphaSort: false });

        this.cameraNode = new Node({ matrix: mat4.create(), camera: new Camera({}) });
    }

    // #cascadeFrames = 0; //used for alternative frame renders for further cascades
        ...
        for(const light of graph.lights) {
            switch(light.type) {
                case LIGHT_TYPES.directional: {
                    ...
                    const splits = ShadowNode.calculateSplitDistances(near, far, settings.lambda, settings.cascades);
                    for(let i = 0; i < splits.length - 1; i++) {
                        // if(this.#cascadeFrames % (i + 1) !== 0) continue;
    ...
    renderShadow(graph, light, layer) {
        ...
        const shadows = this.fbo.attachments.depth;
    ...
    updateFrustum(light, frustum, near, far) {
        ...
        if(light.type === LIGHT_TYPES.spot) {
            ...
            const { center, radius } = ShadowNode.calculateBoundingSphere(frustum, near, far);
