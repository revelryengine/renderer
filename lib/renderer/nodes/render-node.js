...
export class RenderNode {
    ...
    createOutputTextures() {
        ...
        for(const [name, textureInfo] of this.getOutputTextureInfos()) {
            ...
            const { min, mag, wrapS, wrapT, compareFunc, compareMode, array } = { ...defaultParams[textureInfo.attachmentType], ...textureInfo.params };
    ...
    createFramebuffer() {        
        ...
        for(const [name, textureInfo] of this.getOutputTextureInfos()) {
            ...
            const { internalFormat, format, type, array } = { ...defaultParams[textureInfo.attachmentType], ...textureInfo.params };
            ...
            if(target === gl.TEXTURE_2D_ARRAY) {
                gl.texImage3D(target, 0, internalFormat, width, height, 32, 0, format, type, null);
            } else {
    ...
    createRenderbuffers() {
        ...
        let colors = 0;
        for(const [, textureInfo] of this.getOutputTextureInfos()) {
