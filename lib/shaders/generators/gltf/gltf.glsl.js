...
export function generate({ flags, locations }) {
    ...
    if(!flags.shadowPass) {
        fragment = /* glsl */`#version 300 es
            ...
            layout(location=2) out uint g_finalPointId;
            `: ''}

            ${flags.storeMotionVector ? /* glsl */`
            in vec4 v_motionPosition;
            in vec4 v_motionPositionPrev;
            layout(location=1) out vec2 g_finalMotionVector;
            `: ''}
            ...
            void main(void) {
                ${flags.basePassIdOnly ? /* glsl */`
                    g_finalColor      = vec4(0);
                    g_finalPointInfo  = vec4(0);
