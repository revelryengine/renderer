...
export function generate({ flags, locations }) {
    ...
    const vertex = /* glsl */`#version 300 es
        ...
        void main(void){
            ...
            for (int i = 0; i < punctual.shadowCount; ++i) {
                v_ShadowTexcoords[i] = punctual.shadowMatrices[i] * pos;
            }
            `: ''}

            ${flags.storeMotionVectors ? /* glsl */`
            v_motionPosition = gl_Position;
            v_motionPositionPrev = frustum.prevViewProjectionMatrix * modelInfo.matrix * getPosition(modelInfo);
            `: ''}
        }
    `;
    ...
    if(!flags.shadowPass) {
        fragment = /* glsl */`#version 300 es
            ...
            layout(location=1) out vec4 g_finalPointInfo;
            `: ''}

            ${flags.storeMotionVectors ? /* glsl */`
            in vec4 v_motionPosition;
            in vec4 v_motionPositionPrev;
            layout(location=1) out vec4 g_finalMotionVector;
            `: ''}
            ...
            void main(void) {
                ...
                ${flags.storeMotionVectors ? /* glsl */`
                    ...
                    g_finalMotionVector = vec4((a - b), 0.0, 1.0);
                `: ''}
        ...
        fragment = /* glsl */`#version 300 es
            ...
            void main(void) {
                g_finalColor = vec4(vec3(gl_FragCoord.z * 0.5 + 0.5), 1.0);
            }
        `;
    }
