...
export function generate({ input: { prefilter } }) {
    ...
    const fragment = /* glsl */`#version 300 es
        ...
        #pragma revTextureBinding(colorTexture, 0, 2, 1)
        ...
        float computeLod(float pdf) {
            ...
            // https://cgg.mff.cuni.cz/~jaroslav/papers/2007-sketch-fis/Final_sap_0073.pdf
            return 0.5 * log2( 6.0 * size * size / (float(prefilter.sampleCount) * pdf));
        }
        ...
        vec3 filterColor(vec3 N) {
            ...
            for(int i = 0; i < prefilter.sampleCount; ++i) {
                vec4 importanceSample = getImportanceSample(i, N, prefilter.roughness, prefilter.distribution, prefilter.sampleCount);
            ...
            if(weight != 0.0) {
                ...
                color = color / float(prefilter.sampleCount);
            }
    ...
    const fragment = /* glsl */`#version 300 es
        ...
        vec3 LUT(float NdotV, float roughness) {
            ...
            for(int i = 0; i < prefilter.sampleCount; ++i) {
                ...
                vec4 importanceSample = getImportanceSample(i, N, roughness, ${GGX}, prefilter.sampleCount);
            ...
            return vec3(4.0 * A, 4.0 * B, 4.0 * 2.0 * ${M_PI} * C) / float(prefilter.sampleCount);
        }
