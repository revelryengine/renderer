...
export function generate({ flags }) {
    ...
    const code = /* wgsl */`
        ...
        [[group(2), binding(6)]] var transmissionTexture: texture_2d<f32>;
        `: ''}
        ...
        fn applyOcclusion() {
            lightInfo.diffuse   = mix(lightInfo.diffuse,   lightInfo.diffuse   * materialInfo.occlusion, material.occlusionStrength);
            lightInfo.specular  = mix(lightInfo.specular,  lightInfo.specular  * materialInfo.occlusion, material.occlusionStrength);
            lightInfo.sheen     = mix(lightInfo.sheen,     lightInfo.sheen     * materialInfo.occlusion, material.occlusionStrength);
            lightInfo.clearcoat = mix(lightInfo.clearcoat, lightInfo.clearcoat * materialInfo.occlusion, material.occlusionStrength);
        }
        ...
        fn getIBLRadianceLambertian(n: vec3<f32>, v: vec3<f32>, roughness: f32, diffuseColor: vec3<f32>, F0: vec3<f32>, specularWeight: f32) -> vec3<f32> {
            ...
            var F_avg = specularWeight * (F0 + (1.0 - F0) / 21.0);
        ...
        fn getPunctualRadianceTransmission(normal: vec3<f32>, view: vec3<f32>, pointToLight: vec3<f32>, alphaRoughness: f32, f0: vec3<f32>, f90: vec3<f32>, baseColor: vec4<f32>, ior: f32) -> vec3<f32> {
        ...
        fn getIBLVolumeRefraction(n: vec3<f32>, v: vec3<f32>, perceptualRoughness: f32, baseColor: vec4<f32>, f0: vec3<f32>, f90: vec3<f32>, position: vec3<f32>, modelScale: vec3<f32>, viewMatrix: mat4x4<f32>, projMatrix: mat4x4<f32>, ior: f32, thickness: f32, attenuationColor: vec3<f32>, attenuationDistance: f32) -> vec3<f32> {
        ...
        fn applyPunctualLights(){
            ...
            for (var i = 0; i < ${MAX_LIGHT_COUNT}; i = i + 1) {
                ...
                if (NdotL > 0.0 || NdotV > 0.0) {
                    var shadow = 1.0;// apply shadow here
                ...
                ${flags.hasExtension?.KHR_materials_transmission ? /* wgsl */`
                    ...
                    var transmittedLight = intensity * getPunctualRadianceTransmission(n, v, l, materialInfo.alphaRoughness, materialInfo.f0, materialInfo.f90, materialInfo.baseColor, materialInfo.ior);

                    ${flags.hasExtension?.KHR_materials_volume ? /* wgsl */`
                    transmittedLight = applyVolumeAttenuation(transmittedLight, length(transmissionRay), materialInfo.attenuationColor, materialInfo.attenuationDistance);
                    `: ''}

                    lightInfo.transmission = lightInfo.transmission + materialInfo.transmissionFactor * transmittedLight;
                `: ''}
            }
        }
        ...
        fn applyTransmission() {
            var n = normalInfo.n;
            var v = normalInfo.v;
            ${flags.hasExtension?.KHR_materials_transmission || flags.hasExtension?.KHR_materials_volume ? /* wgsl */`
                lightInfo.transmission = lightInfo.transmission + materialInfo.transmissionFactor * getIBLVolumeRefraction(
                    n, v,
