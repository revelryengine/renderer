...
export function generate({ flags }) {
    ...
    const code = /* glsl */`
        ...
        #pragma revTextureBinding(transmissionTexture, 2, 6, 0)
        ...
        vec3 getPunctualRadianceTransmission(vec3 normal, vec3 view, vec3 pointToLight, float alphaRoughness, vec3 f0, vec3 f90, vec4 baseColor, float ior) {
        ...
        vec3 getIBLVolumeRefraction(vec3 n, vec3 v, float perceptualRoughness, vec4 baseColor, vec3 f0, vec3 f90, vec3 position, vec3 modelScale, mat4 viewMatrix, mat4 projMatrix, float ior, float thickness, vec3 attenuationColor, float attenuationDistance) {
        ...
        void applyPunctualLights() {
            ...
            for (int i = 0; i < ${MAX_LIGHT_COUNT}; i = i + 1) {
                ...
                if (NdotL > 0.0 || NdotV > 0.0) {
                    float shadow   = 1.0;// apply shadow here
        ...
        void applyTransmission() {
            vec3 n = normalInfo.n;
            vec3 v = normalInfo.v;
            ${flags.hasExtension?.KHR_materials_transmission || flags.hasExtension?.KHR_materials_volume ? /* glsl */`
