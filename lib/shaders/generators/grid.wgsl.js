...
const frustumUniformBlock = generateUniformBlock(Frustum, 0, 0);

export function generate({ input: { grid } }) {
    const gridUniformBlock = generateUniformBlock(grid, 1, 0);
    ...
    const vertex = /* wgsl */`
        ...
        fn main([[builtin(vertex_index)]] VertexIndex : u32) -> VertexOutput {
            ...
            out.gl_Position = frustum.viewProjectionMatrix * vec4<f32>(pos, 1.0);
            return out;
        }
    `;
    ...
    const fragment = /* wgsl */`
        ...
        let min_pixels_between_cells = 1.0;
        ...
        fn main(in: VertexOutput) -> FragmentOutput {
            ...
            // var c = lod2_a > 0.0 ? grid.colorThick : lod1_a > 0.0 ? mix(grid.colorThick, grid.colorThin, lod_fade) : grid.colorThin;
            var c = select(select(grid.colorThin, mix(grid.colorThick, grid.colorThin, lod_fade), lod1_a > 0.0), grid.colorThick, lod2_a > 0.0);
