...
export class GLTFShader extends Shader {
    ...
    async init() {
        const { primitive, material, frontFace, sampleCount } = this.input;
        ...
        if(this.flags.basePass) {
            targets.push({ format: 'rgba32float' });
            targets.push({ format: 'r32uint' });
        }
...
export class GLTFBaseShader extends GLTFShader {
    getFlags(...args) {
        ...
        return {
            ...flags,
            basePass        : true,
            basePassIdOnly  : flags.isBlend || flags.useTransmission || (!settings.transmission.enabled && !settings.ssao.enabled),
            useLinear       : true,
