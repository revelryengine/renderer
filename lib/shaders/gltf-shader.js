...
import generateAlphaWGSL from './generators/gltf/alpha-composite.wgsl.js';
import generateAlphaGLSL from './generators/gltf/alpha-composite.glsl.js';
...
export class GLTFShader extends Shader {
    ...
    async init() {
        const { flags, input: { primitive, material, frontFace, sampleCount } } = this;
        ...
        const targets = [
            ...
            //         }
            //     } : undefined,
            //     writeMask: flags.colorWriteMask,
            // } : null,
            ...(flags.outputColor ? [
                { format: 'rgba8unorm',  blend, writeMask:  flags.isBlend ? 0 : flags.colorWriteMask },
                { format: 'rgba16float', blend, writeMask: !flags.isBlend ? 0 : flags.colorWriteMask },
                { format: 'r8unorm',     blend, writeMask: !flags.isBlend ? 0 : flags.colorWriteMask },
            ] : [null, null, null]),
            flags.outputPoint  ? { format: 'rgba32float' } : null,
            flags.outputId     ? { format: 'r32uint'     } : null,
            flags.outputMotion ? { format: 'rg16float'   } : null,
        ];
    ...
    getFlags() {
        ...
        const flags = {
            ...
            outputColor       : true,
            outputMotion      : settings.temporal,
...
export class GLTFBaseShader extends GLTFShader {
    getFlags(...args) {
        ...
        return {
            ...
            outputColor    : settings.transmission.enabled,
            outputMotion   : false,
            outputPoint    : settings.ssao.enabled,
            outputId       : settings.passiveInput.enabled,
            colorWriteMask : flags.hasExtension.KHR_materials_transmission ? 0 : 0xF,
        }
    }
}
...
export class GLTFAlphaCompositeShader extends Shader {
    static wgsl = generateAlphaWGSL;
    static glsl = generateAlphaGLSL;
