...
export class GLTFShader extends Shader {
    ...
    async init() {
        const { primitive, material, multisample } = this.input;
    ...
    getFlags() {
        const { primitive, material, transmission, settings, basePass = false } = this.input;
    ...
    static getShaderKey(primitive, material) {
        ...
        return keys.get(material) || keys.set(material, { id: this.#shaderKeyIds++ }).get(material);
    }
