...
import { BUFFER_USAGE, PRIMITIVE_MODES, TEXTURE_USAGE, SHADER_STAGE } from '../constants.js';
...
export class GLTFShader extends Shader {
    ...
    constructor(gal, input) {
        ...
        const { primitive, multisample, nodeLayout } = input;
                    ...
                    attributes: [
                        {
                            ...
                            offset: 0,
                            format: gal.getAccessorVertexFormat(attribute),
                        }
                    ]
                });
            }
        ...
        this.renderPipeline = gal.device.createRenderPipeline({
            label: 'GLTF',
            layout: gal.device.createPipelineLayout({ bindGroupLayouts }),
            vertex:   {
    ...
    getFlags(input) {
        const { primitive, transmission, linear } = input;

        const flags = {
            hasAttr         : Object.fromEntries(Object.entries(primitive.attributes).map(([name, value]) => [name, !!value])),
            hasColor0Vec3   : primitive.attributes.COLOR_0?.type === 'VEC3',

            useEnvironment  : input.settings?.environment?.enabled,
            usePunctual     : input.settings?.punctual?.enabled,
            useTransmission : !!transmission,
            useLinear       : !!linear,
            tonemap         : input.settings?.tonemap,
        ...
        const material = this.gal.getMaterialUBO(primitive.getActiveMaterial(this.gal));
        Object.assign(flags, {
    ...
    createBindGroup() {
                ...
                for(const [name, accessor] of Object.entries(target)) {
                    if(locations.targets[name] !== undefined) {
                        ...
                        if(accessor.type === 'VEC2' || accessor.type === 'VEC3') {
                            const padded = new Float32Array(Math.pow(textureSize, 2) * 4);
        ...
        const materialUBO = this.gal.getMaterialUBO(primitive.getActiveMaterial(this.gal));
        ...
        for(const [name, loc] of Object.entries(locations.textures)) {
            ...
            const { texture, sampler } = this.gal.getTextureFromGLTF(info);
            layoutEntries.push(
                {
                    binding: loc,
                    visibility: SHADER_STAGE.FRAGMENT,
    ...
    static getShaderKey(primitive) {
        ...
        const material = primitive.getActiveMaterial();
