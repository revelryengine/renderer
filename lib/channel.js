...
export class Channel extends GLTFProperty {
   ...
   */
  dereference(root, animation) {
    this.dereferenceFromCollection('sampler', animation.samplers);
    this.target.dereference(root);
   ...
   * @param {Animation} animation - The parent animation object.
   */
  rereference(root, animation) {
    this.rereferenceFromCollection('sampler', animation.samplers);
    this.target.rereference(root);
