...
export class Renderer {
  ...
  constructor(target, { settings, xrCompatible = false, scaleFactor = 1, pipeline } = {}) {
    this.settings = Renderer.mergeSettings(settings);
    ...
    this.defaultLights = [
      new KHRLightsPunctualLight({ type: 'directional', intensity: 2 }).getUniformStruct(
        mat4.fromRotation(mat4.create(), Math.PI / 4, [-1, 1, 0])
  ...
  static mergeSettings(settings = {}) {
    ...
    for(const [name, value] of Object.entries(DEFAULT_SETTINGS)) {
      if(value instanceof Object) output[name] = { ...DEFAULT_SETTINGS[name], ...settings[name] }
      else output[name] = settings[name] !== undefined ? settings[name] : DEFAULT_SETTINGS[name];
    }
    return output;
  }
  ...
  render(scene, cameraNode, output = this.context.canvas) {
    ...
    const { context: gl, pipeline, frustum, environment, defaultLights } = this;
    ...
    { // UBO setup
      const { environmentUBO } = this.pipeline;
      
      environmentUBO.set({
        u_Exposure: 1,
        u_FogColor: this.settings.fog.color,
        u_FogRange: this.settings.fog.range,
      });
