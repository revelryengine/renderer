...
export class Renderer {
    ...
    #run({ graph, frusta, path }) {
        const commandEncoder = this.gal.device.createCommandEncoder();

        const renderPath = this.renderPaths[path];
        ...
        for(const node of renderPath.prePath) {
            node.run(commandEncoder, { graph, frustum, settings });
        }
        ...
        for(let i = 0; i < frusta.length; i++) {
            ...
            for(const node of renderPath.path) {
                node.run(commandEncoder, { graph, frustum, instances, settings });
            }
        }
    ...
    render(graph, frustum, path = 'standard') {
        this.#run({ graph, frusta: [frustum], path });
    }
    ...
    async getNodeAtPoint(graph, input, path = 'standard') {
        const { texture } = this.renderPaths[path].nodes.base?.output.id;
        if(!texture) return;
