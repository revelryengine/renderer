...
export class Renderer {
    ...
    reconfigure() {
        ...
        if(this.settings.passiveInput.enabled || this.settings.transmission.enabled || this.settings.ssao.enabled) {
            nodes.base ??= new BaseNode(this);
        } else {
            nodes.base = nodes.base?.destroy();
        }

        if(this.settings.transmission.enabled) {
        ...
        if(this.settings.audio.enabled) {
            this.#pre.push(nodes.audio ??= new AudioNode(this));
        } else {
        ...
        if(this.settings.environment.enabled) {
            this.#pre.push(nodes.environment ??= new EnvironmentNode(this));
        ...
        if(this.settings.punctual.enabled) {
            this.#pre.push(nodes.punctual ??= new PunctualNode(this));
    ...
    render(graph, frustum, input) {
        this.#run({ graph, frusta: [frustum], input });

        if(input) {
            return this.#getNodeAtPoint({ graph, frustum, input });
        }
    }
    ...
    async #getNodeAtPoint({ graph, input }) {
        const { texture } = this.#nodes.base?.output.id;
        if(!texture) return;

        const x = input.x * texture.width;
        const y = input.y * texture.height;
