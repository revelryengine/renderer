...
export class Graph {
    ...
    updateBindGroup(settings) {
        const { model, joint, morph } = this.#buffers;
    ...
    getState(node) {
        ...
        if (!state) {
            ...
            if (!node.mesh) {
                ...
                const { index, block } = this.#buffers.model.createMatrixViewBlock(2);
                state.worldTransform  = block[0].matrixView;
                state.modelMatrix     = block[0].matrixView;
    ...
    getSkinState(skin) {
        ...
        if(!state) {
            const { index, block } = this.#buffers.joint.createMatrixViewBlock(skin.joints.length * 2);
    ...
    getMorphState(weights) {
        ...
        if(!state) {
            const { index, block } = this.#buffers.morph.createMatrixViewBlock(Math.ceil(weights.length / 16))
    ...
    updateNode(node) {
        ...
        if(node.extensions?.KHR_audio) {
            if (this.audio.indexOf(node) === -1) {
                this.audio.push(node);
            }
        }

        const gameObjectId = node.extensions?.REV_game_object?.id ?? (parent && this.getState(parent).gameObjectId);
        if(gameObjectId) {
        ...
        if (mesh) {
            ...
            for (const primitive of mesh.primitives) {
                ...
                const instance = { index, node, primitive, worldTransform, aabb, frontFace };
     ...
     */
    generateInstances(frustum, sortAlpha) {
        const gameObjects = {};
        ...
        let bufferSize = 0, lastPrimitive = null, lastFrontFace, currentBlock = null;
        const processInstance = (blocks, instance) => {
            if(!frustum.containsAABB(instance.aabb)) return;

            const { index, primitive, frontFace, node } = instance;
            const { morph, skin, gameObjectId } = this.getState(node);

            if(primitive !== lastPrimitive || frontFace !== lastFrontFace) {
                currentBlock = { primitive, frontFace, instances: [] };
                blocks.push(currentBlock);
            }

            if(gameObjectId !== undefined) {
                gameObjects[gameObjectId] ??= [];
                gameObjects[gameObjectId].push({ block: currentBlock, index: currentBlock.instances.length / 4 });
            }

            currentBlock.instances.push(index, skin?.index ?? 0, morph?.index ?? 0, node.$id);
            bufferSize += 4;
        ...
        if(sortAlpha) {
            ...
            this.instances.alpha.sort((a, b) => getDepth(a) - getDepth(b))
        }
        ...
        for(const instance of this.instances.alpha){
    ...
    sort() {
        const byPrimitiveId = (a, b) => {
            if (a.frontFace < b.frontFace) {
                return -1;
            } else if (a.frontFace > b.frontFace) {
                return 1;
            }

            if (a.primitive.$id < b.primitive.$id) {
                return -1;
