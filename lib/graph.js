...
import { MatrixTBO, TBOMirror } from './tbo.js';
...
export class Graph {
    ...
    updateBindGroup() {
        ...
        if(this.gal.settings.temporal) {
            this.#temporalBuffers = this.#temporalBuffers || {
                model: new TBOMirror(this.modelBuffer),
                joint: new TBOMirror(this.jointBuffer),
                morph: new TBOMirror(this.morphBuffer),
            };

            entries.push(
                //modelMatrixHistory
                { binding: 4, resource: this.#temporalBuffers.model.texture.createView({ dimension: '2d-array' }) },
                //jointMatrixHistory
                { binding: 5, resource: this.#temporalBuffers.joint.texture.createView({ dimension: '2d-array' }) },
                //morphMatrixHistory
                { binding: 6, resource: this.#temporalBuffers.morph.texture.createView({ dimension: '2d-array' }) },
            );
        } else {
            if(this.#temporalBuffers) {
                this.#temporalBuffers.model.texture.destroy();
                this.#temporalBuffers.joint.texture.destroy();
                this.#temporalBuffers.morph.texture.destroy();
            }
            
            this.#temporalBuffers = null;
            /**
             * It's just simpler to duplicate the original texture buffers rather than change the bind group layout in various places.
             */
            entries.push(
                { binding: 4, resource: entries[1].resource },
                { binding: 5, resource: entries[2].resource },
                { binding: 6, resource: entries[3].resource },
            );
        }
    ...
    mirrorHistory(commandEncoder) {
        if(this.#temporalBuffers) {
            this.#temporalBuffers.model.mirror(commandEncoder);
            this.#temporalBuffers.joint.mirror(commandEncoder);
            this.#temporalBuffers.morph.mirror(commandEncoder);
        }
    }
