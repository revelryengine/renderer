...
import { BUFFER_USAGE, LIGHT_TYPES, SHADER_STAGE } from './constants.js';
...
export class Graph {
    ...
    static bindGroupLayout = {
        entries: [
            //modelMatrix
            { binding: 0, visibility: SHADER_STAGE.VERTEX, texture: { viewDimension: '2d-array', sampleType: 'unfilterable-float' } },
            //jointMatrix
    ...
    updateBindGroup() {
        this.bindGroup = this.gal.device.createBindGroup({
            ...
            entries: [
                //modelMatrix
                { binding: 0, resource: this.modelBuffer.texture.createView({ dimension: '2d-array' }) },
                //jointMatrix
                { binding: 1, resource: this.jointBuffer.texture.createView({ dimension: '2d-array' }) },
                //morphMatrix
                { binding: 2, resource: this.morphBuffer.texture.createView({ dimension: '2d-array' }) },
            ],
        });
    }
    ...
    generateInstances(frustum) {
        ...
        this.gal.device.queue.writeBuffer(frustumInstances.buffer, 0, data);
