...
export class Primitive extends GLTFProperty {
  ...
  constructor(primitive) {
    ...
    Object.defineProperty(this, '$programs', { value: new WeakMap() });
  }
  ...
  dereference(root) {
    ...
    if (this.indices) this.indices.bufferView.target = WebGLRenderingContext.ELEMENT_ARRAY_BUFFER;
  }
  ...
  createProgram(context, { brdfTexture, diffuseTexture, specularTexture } = {}, extensions = {}) {
    let program = this.$programs.get(context);
    if (program) return program;

    const { attributes, targets, material = {} } = this;
    const { pbrMetallicRoughness = {} } = material;

    program = new PBRProgram(context, {
  ...
  getProgram(context, environment, extensions) {
    return this.$programs.get(context) ||
      this.$programs.set(context, this.createProgram(context, environment, extensions)).get(context);
  }
