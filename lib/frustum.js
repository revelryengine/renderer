...
export class Frustum extends UBO {
     ...
     * @return {vec3}
     */
    project(point, flipY = false) {
        const projected =this.getProjectedPoint(point);
        vec3.scale(point, projected, 1 / projected[3]);
        vec2.multiply(point, point, [this.width / 2, this.height / 2 * (flipY ? -1 : 1)]);
        return point;
    }
    ...
    getPerspectiveDivide(point) {
        const projected = vec4.transformMat4(vec4.create(), vec4.fromValues(...point, 1), this.viewProjectionMatrix);
        return projected[3];
    }

    getProjectedPoint(point) {
        return vec4.transformMat4(vec4.create(), vec4.fromValues(...point, 1), this.viewProjectionMatrix);
    }
