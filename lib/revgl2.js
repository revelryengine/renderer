...
export class RevGL2 extends RevGAL {
    ...
    generateShaders(shaders, input) {
        ...
        const { vertex, fragment, flags, cacheKey } = glsl; /** @todo use cacheKey */
        const vertShader = this.#createShader(GL.VERTEX_SHADER, vertex);
        const fragShader = this.#createShader(GL.FRAGMENT_SHADER, fragment);
        return { vertShader, fragShader, flags };
    }

    async checkCompilation(module) {
        if (!this.device[_].context.getShaderParameter(module, this.device[_].context.COMPILE_STATUS)) {
            const log = this.device[_].context.getShaderInfoLog(module);
            const src = this.device[_].context.getShaderSource(module);
