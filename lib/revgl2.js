...
class RevGL2RenderPassEncoder extends RevGL2ProgrammablePassEncoder {
    ...
    #createFramebuffer(descriptor) { 
        ...
        for(let i = 0; i < colorAttachments.length; i++) {
            ...
            const { glTexture, glTarget        } = texture[_];
            const { depthOrArrayLayers, format } = texture;
            ...
            if(loadOp !== 'load') gl[TEXTURE_FORMAT[format].webgl2.clearMethod](gl.COLOR, i, clearValue);
        }
        ...
        if(sampleCount > 1) {
            ...
            for(let i = 0; i < colorAttachments.length; i++) {
                if(!colorAttachments[i]) {
                    renderbuffers.push({ attachment: null });
                    continue;
                };
    ...
    setViewport(x, y, width, height, minDepth, maxDepth) {
        this[_].command_encoder[_].command_list.push(() => {
            ...
            Object.assign(this[_].viewport, { x, y: attachment_size.height - height - y, width, height, minDepth, maxDepth });
        });
    }
