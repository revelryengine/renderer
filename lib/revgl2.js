...
class RevGL2Device extends RevGL2ObjectBase {
    constructor(context, descriptor) {
        ...
        for(const ext of extensions) {
            ...
            if(!glExtensions[ext] && required[ext]) {
...
class RevGL2RenderPassEncoder extends RevGL2ProgrammablePassEncoder {
    ...
    #createFramebuffer(descriptor) { 
        ...
        for(let i = 0; i < colorAttachments.length; i++) {
            const { view, loadValue     } = colorAttachments[i];
            ...
            gl.clearBufferfv(gl.COLOR, i, loadValue);
        }

        if(depthStencilAttachment) {
            const { view, depthLoadValue, stencilLoadValue } = depthStencilAttachment;
            const { texture, descriptor } = view[_];
            ...
            gl.clearBufferfi(gl.DEPTH_STENCIL, 0, depthLoadValue, stencilLoadValue);
        }
    ...
    #createRenderbuffers({ colorAttachments = [], depthStencilAttachment }) {
        ...
        if(sampleCount > 1) {
            ...
            if(depthStencilAttachment) {
                const { view, resolveTarget } = depthStencilAttachment;
                const { descriptor } = view[_];
    ...
    #startDraw(){
        ...
        const { blend } = descriptor.fragment.targets[0];
        if(blend) {
            gl.enable(gl.BLEND);
 ...
 */
 class RevGL2RenderPipeline extends RevGL2ObjectBase {
    constructor(device, descriptor) {
        ...
        this[_].glProgram       = this.#linkShaders(descriptor);
        this[_].glBindLocations = this.#getBindLocations(this[_].glProgram, descriptor);
    }

    #linkShaders({ fragment, vertex }) {
        const gl = this[_].device[_].context;

        const program = gl.createProgram();
        gl.attachShader(program, vertex.module);
        gl.attachShader(program, fragment.module);
    ...
    #getBindLocations(program, { layout, fragment, vertex }) {
        ...
        const vertSrc = gl.getShaderSource(vertex.module);
        const fragSrc = gl.getShaderSource(fragment.module);
...
class RevGL2Texture extends RevGL2ObjectBase {
    ...
    #createTexture({ target, size, format, mipLevelCount = 1 }) {
        ...
        if (ext){
            gl.texParameterf(target, ext.TEXTURE_MAX_ANISOTROPY_EXT, gl.getParameter(ext.MAX_TEXTURE_MAX_ANISOTROPY_EXT));
        }
...
class RevGL2Sampler extends RevGL2ObjectBase {
    ...
    #createSampler(descriptor) {
        ...
        if(compare) gl.samplerParameteri(sampler, GL.TEXTURE_COMPARE_FUNC, COMPARE_FUNC[compare]);
...
export class RevGL2 extends RevGAL {
    ...
    generateMipmap(commandEncoder, texture) {
        const { glTexture, glTarget } = texture[_];
        const gl = this.device[_].context;
    ...
    generateShaders(shaders, input) {
        let { glsl } = shaders;
        if(typeof glsl === 'function') {
            glsl = glsl(input);
        }
        const { cacheKey } = input;
        const { vertex, fragment, flags } = glsl;
        const vertShader = this.#vertShaders.get(cacheKey) || this.#vertShaders.set(cacheKey, this.#createShader(GL.VERTEX_SHADER, vertex)).get(cacheKey);
        const fragShader = this.#fragShaders.get(cacheKey) || this.#fragShaders.set(cacheKey, this.#createShader(GL.FRAGMENT_SHADER, fragment)).get(cacheKey);
    ...
    /**
     * @todo refactor this to use createRenderPipelineAsync instead
     * 
     */
    checkCompilation(module) {
        const gl  = this.device[_].context;
        
        if (!gl.getShaderParameter(module, gl.COMPILE_STATUS)) {
            const log = gl.getShaderInfoLog(module);
            const src = gl.getShaderSource(module);
    ...
    #createShader(type, source) {
        const shader = this.device[_].context.createShader(type);
        this.device[_].context.shaderSource(shader, source);
        this.device[_].context.compileShader(shader);
        return shader;
    }
