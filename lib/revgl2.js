...
class RevGL2Device extends RevGL2ObjectBase {
    constructor(context, descriptor) {
        ...
        this.lost = new Promise((resolve) => {
            context.canvas.addEventListener('webglcontextlost', (event) => {
                resolve({ message: event.statusMessage, reason: this.#destroyed ? 'destroyed' : undefined });
            }, { once: true});
        });
    }
    ...
    createQuerySet(descriptor) {
        return new RevGL2QuerySet(this, descriptor);
    }


}

class RevGL2QuerySet extends RevGL2ObjectBase {
...
class RevGL2Queue extends RevGL2ObjectBase {
    ...
    writeTexture(destination, data, dataLayout, size) {
        ...
        const { webgl2 } = TEXTURE_FORMAT[texture.format];
...
class RevGL2RenderPassEncoder extends RevGL2ProgrammablePassEncoder {
    ...
    #createFramebuffer(descriptor) { 
        ...
        for(let i = 0; i < colorAttachments.length; i++) {
            if(!colorAttachments[i]) continue;
            const { view                } = colorAttachments[i];
    ...
    #beginRenderPipeline() {
        ...
        for(let i = 0; i < colorAttachments.length; i++) {
            ...
            if(loadOp !== 'load') {
                oes.colorMaskiOES(i, 1, 1, 1, 1);
                gl[TEXTURE_FORMAT[texture.format].webgl2.clearMethod](gl.COLOR, i, clearValue);
            }
        }

        if(depthStencilAttachment) {
            const { depthLoadOp, depthClearValue, stencilLoadOp, stencilClearValue } = depthStencilAttachment;
    ...
    #createRenderbuffers({ colorAttachments = [], depthStencilAttachment }) {
        const gl = this[_].device[_].context;
        const { sampleCount   } = (colorAttachments[0] ?? depthStencilAttachment).view[_].texture;
        ...
        if(sampleCount > 1) {
            ...
            for(let i = 0; i < colorAttachments.length; i++) {
                if(!colorAttachments[i]) {
                    renderbuffers.colors.push({ attachment: null });
                    continue;
                };
                ...
                renderbuffers.colors.push({ attachment, resolveTarget, glRenderbuffer });
            }
    ...
    #resolveRenderbuffers() {
        ...
        if(depth) {
            gl.deleteRenderbuffer(depth.glRenderbuffer);
        }
    }
    ...
    #startDraw(){
        const gl = this[_].device[_].context;
        const { descriptor, writesDepth, glProgram, glDrawBuffers } = this[_].pipeline[_];
        ...
        const { OES_draw_buffers_indexed: oes } = this[_].device[_].glExtensions;
        for(let i = 0; i < descriptor.fragment.targets.length; i++) {
            ...
            oes.colorMaskiOES(i, writeMask & 0x1, writeMask & 0x2, writeMask & 0x4, writeMask & 0x8);
        }
    ...
    beginOcclusionQuery(queryIndex) {
        const gl = this[_].device[_].context;
        this[_].command_encoder[_].command_list.push(() => {
            gl.beginQuery(gl.ANY_SAMPLES_PASSED, this[_].occlusion_query_set[_].glQueries[queryIndex]);
        });
    }
...
class RevGL2RenderPipeline extends RevGL2ObjectBase {
    constructor(device, descriptor, async) {
        ...
        this[_].glDrawBuffers = descriptor.fragment.targets.map((target, i) => target && (target.writeMask ?? 0xF) && GL.COLOR_ATTACHMENT0 + i);
...
export class RevGL2TextureView extends RevGL2ObjectBase {
    ...
    static #resolveDefaultViewDescriptor(texture, { format, dimension, aspect = 'all', baseMipLevel = 0, mipLevelCount, baseArrayLayer = 0, arrayLayerCount } = {}) {
        ...
        resolved.format = resolved.format ?? texture.format;
        resolved.mipLevelCount = resolved.mipLevelCount ?? texture.mipLevelCount - baseMipLevel;

        if(!resolved.dimension) {
            if(texture.dimension === '1d') resolved.dimension = '1d';
            if(texture.dimension === '2d') resolved.dimension = '2d';
            if(texture.dimension === '3d') resolved.dimension = '3d';
        }
...
class RevGL2Texture extends RevGL2ObjectBase {
    ...
    constructor(device, descriptor) {
        ...
        const { size: { width, height, depthOrArrayLayers = 1 }, format, usage, mipLevelCount = 1, sampleCount = 1, dimension = '2d', array = false, cubemap = false } = descriptor;
        this.#descriptor = { size: { width, height, depthOrArrayLayers }, format, usage, mipLevelCount, sampleCount, dimension };
        
        const target = this.#getTarget({ dimension, depthOrArrayLayers, array, cubemap });
    ...
    #getTarget({ dimension, depthOrArrayLayers, array, cubemap }) {
        if(dimension === '3d') {
            return GL.TEXTURE_3D;
        } else if(array || cubemap || depthOrArrayLayers > 1) {
            if(cubemap) {
...
export class RevGL2 extends RevGAL {
    ...
    constructor(target) {
        ...
        super(context, new RevGL2Device(context), 'rgba8unorm');
    }
    ...
    async readTexture({ texture, origin, size, mipLevel = 0 }) {
        ...
        const { bytes, webgl2 } = TEXTURE_FORMAT[texture.format];
        const { format, type } = webgl2;

        const { width, height, depthOrArrayLayers = 1 } = size ?? texture;
        const { x = 0, y = texture.height, z = 0 } = origin ?? {};
    ...
    async resolveOcclusionQuerySet(querySet) {
        ...
        return Promise.all(glQueries.map(async (query) => {
