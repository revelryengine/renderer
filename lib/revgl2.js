...
export class RevGL2RenderPassEncoder extends RevGL2ObjectBase {
    ...
    #beginRenderPipeline() {
        ...
        gl.drawBuffers(/** @type {unknown[]}*/(colorAttachments).map((_, i) => gl.COLOR_ATTACHMENT0 + i));
...
export class RevGL2Texture extends RevGL2ObjectBase {
    #descriptor;
    /**
     * @param {GPUTextureDescriptor & { array?: boolean, cubemap?: boolean }} descriptor
     * @param {RevGL2Device} device
     */
    constructor(descriptor, device) {
        super(descriptor, device);
    ...
    /**
     * @param {{ dimension: GPUTextureDimension, depthOrArrayLayers: number, array: boolean, cubemap: boolean }} options
     */
...
export class RevGL2ShaderModule extends RevGL2ObjectBase {
    ...
    async getCompilationInfo() {
        ...
        if (!gl.getShaderParameter(this[_].glShader, gl.COMPILE_STATUS)) {
            ...
            if(!log || !src) throw new Error('Failed to collect shader info log and source');
            const [,linePos = -1, lineNum = -1, message = '???'] = log.match(/ERROR: (\d+):(\d+):(.*)/) ?? [];
            return { messages: [{ type: /** @type {GPUCompilationMessageType} */('error'), linePos: Number(linePos), lineNum: Number(lineNum), message, src, offset: 0, length: 0 }] };
        }
        return { messages: [] };
    }
}
