...
export class KHRMaterialsSheenMaterial extends GLTFProperty {
  ...
  defineMaterial(PBRProgram, defines) {
    ...
    if(this.sheenColorTexture) PBRProgram.defineTexture(defines, this.sheenColorTexture, 'sheenColorTexture');
    if(this.sheenRoughnessTexture) PBRProgram.defineTexture(defines, this.sheenRoughnessTexture, 'sheenRoughnessTexture');
  }

  applyMaterial(program, context) {
    if(this.sheenColorTexture) program.applyTexture(context, this.sheenColorTexture, 'sheenColorTexture');
    if(this.sheenRoughnessTexture) program.applyTexture(context, this.sheenRoughnessTexture, 'sheenRoughnessTexture');
    
    program.uniforms.set('u_SheenColorFactor', this.sheenColorFactor);
    program.uniforms.set('u_SheenRoughnessFactor', this.sheenRoughnessFactor);
  }
}
