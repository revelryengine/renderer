...
export class KHRMaterialsTransmissionMaterial extends GLTFProperty {
  ...
  applyMaterial(program, context, _, graph) {
    ...
    const { width, height } = graph.passes.base;

    const opaqueTexture = graph.passes.base.textures?.color;
    
    if(opaqueTexture) {
      program.uniforms.set('u_TransmissionFramebufferSampler', opaqueTexture);
      program.uniforms.set('u_TransmissionFramebufferSize', [width, height]);
    }
  }
}
