...
export class KHRMaterialsPBRSpecularGlossinessMaterial extends GLTFProperty {
  ...
  defineMaterial(PBRProgram, defines) {
    ...
    defines['MATERIAL_METALLICROUGHNESS'] = null;

    if(this.diffuseTexture) PBRProgram.defineTexture(defines, this.diffuseTexture, 'diffuseTexture');
    if(this.specularGlossinessTexture) PBRProgram.defineTexture(defines, this.specularGlossinessTexture, 'specularGlossinessTexture');
  }

  applyMaterial(program, context) {
    if(this.diffuseTexture) program.applyTexture(context, this.diffuseTexture, 'diffuseTexture');
    if(this.specularGlossinessTexture) program.applyTexture(context, this.specularGlossinessTexture, 'specularGlossinessTexture');

    program.uniforms.set('u_DiffuseFactor',    this.diffuseFactor);
    program.uniforms.set('u_SpecularFactor',   this.specularFactor);
    program.uniforms.set('u_GlossinessFactor', this.glossinessFactor);
  }
}
