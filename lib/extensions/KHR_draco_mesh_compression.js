...
export class KHRDracoMeshCompressionPrimitive extends GLTFProperty {
   ...
   * @param {Object} parent - The parent object this extension belongs to.
   */
  constructor(khrDracoMeshCompressionPrimitive, parent) {
  ...
  async load() {
    ...
    decoderBuffer.Init(new Uint8Array(arrayBuffer, byteOffset, byteLength), byteLength);
    ...
    for (const [name, attributeId] of Object.entries(this.attributes)) {
      const accessor = this._parent.attributes[name];
      ...
      draco.destroy(dracoArray);
		...
		if (geometryType === draco.TRIANGULAR_MESH) {
      const accessor = this._parent.indices;
