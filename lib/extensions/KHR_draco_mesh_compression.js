import { wasm, wrapper } from '../../vendor/draco-decoder.js';
...
function worker() {
  /* global DracoDecoderModule wasm64 */
  ...
  const wasmBinary = Uint8Array.from(atob(wasm64), c => c.charCodeAt(0));
...
export class KHRDracoMeshCompressionPrimitive extends GLTFProperty {
  ...
  async load(abortCtl) {
    ...
    await Promise.all([
      buffer.loadOnce(abortCtl), this._primitive.indices.loadOnce(abortCtl),
      ...Object.values(this._primitive.attributes).map(accessor => accessor.loadOnce(abortCtl)),
    ]);
