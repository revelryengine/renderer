...
export class KHRLightsPunctualLight extends NamedGLTFProperty {
  ...
  getUniformStruct(worldTansform) {
    ...
    if(spot) {
      this.#struct.innerConeCos = Math.cos(spot.innerConeAngle);
      this.#struct.outerConeCos = Math.cos(spot.outerConeAngle);
    }

    if(this.type === 'directional' || this.type === 'spot') {
      this.#struct.direction = vec3.fromValues(0.0, 0.0, -1.0);
      if(worldTansform){
        const rotation = quat.create();
        mat4.getRotation(rotation, worldTansform);
        quat.normalize(rotation, rotation);
        vec3.transformQuat(this.#struct.direction, this.#struct.direction, rotation);
      }      
    }
    return this.#struct;
  }
