...
export class Camera extends NamedGLTFProperty {
   ...
   * @param {mat4} [out=mat4.create()] - The output matrix.
   ...
   */
  getProjectionMatrix(outMatrix = mat4.create(), viewportWidth = 1, viewportHeight = 1) {
    if (this.type === 'perspective') {
      const { aspectRatio = viewportWidth / viewportHeight, yfov, znear, zfar } = this.perspective;
      outMatrix[0] = 1 / (aspectRatio * Math.tan(0.5 * yfov));
      outMatrix[5] = 1 / Math.tan(0.5 * yfov);
      outMatrix[10] = zfar ? ((zfar + znear) / (znear - zfar)) : -1;
      outMatrix[11] = -1;
      outMatrix[14] = zfar ? ((2 * zfar * znear) / (znear - zfar)) : -2 * znear;
      outMatrix[15] = 0;
    } else {
      const { xmag, ymag, znear, zfar } = this.orthographic;
      outMatrix[0] = 1 / xmag;
      outMatrix[5] = 1 / ymag;
      outMatrix[10] = 2 / (znear - zfar);
      outMatrix[14] = (zfar + znear) / (znear - zfar);
    }
    return outMatrix;
  }
}
