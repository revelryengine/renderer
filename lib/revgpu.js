...
export class RevGPU extends RevGAL {
    ...
    constructor(target, device) {
        ...
        super(context, device, navigator.gpu.getPreferredCanvasFormat());
    }
        ...
        for(let layer = 0; layer < depthOrArrayLayers; layer++) {
            for (let level = 0; level < mipLevelCount - 1; level++) {
                ...
                passes.push({
                    ...
                    shader: new ResampleShader(this, { label: `createMipmapGenerator ${layer}`, view: srcView, format, viewDimension, minFilter: 'linear' }),
                })
            }
        }
    ...
    createDepthTextureResolver({ source, destination }) {
        ...
        const shader = new ResampleShader(this, { label: 'createDepthTextureResolver', view: srcView, format: source.format, viewDimension: '2d', multisampled: true });
