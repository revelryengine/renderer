  ...
  updateNode(node) {
    ...
    if(mesh) {
      ...
      for(const primitive of mesh.primitives) {
        ...
        const p    = { node, mesh, primitive, worldTransform, aabb };
    ...
    if(skin) {
      /**
       * @todo optimize this, it's pretty inefficient
       */
      if(!this.skins.find(s => node === s.node)){
  ...
  updateSkins() {
    for(const { skin, node } of this.skins) {
      ...
      for (let i = 0; i < skin.joints.length; i++) {
        ...
        state.jointMatrices.push(jointMatrix);
        state.jointNormalMatrices.push(jointNormalMatrix);
        // for (let j = 0; j < 16; j++){
