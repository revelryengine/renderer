...
export class Graph {
   ...
   */
  getState(node) {
    let state = this.state.get(node); //weakmap is really slow, come back to this
    if(!state) {
      state = {
  ...
  updateNode(node) {
    ...
    for (const n of node.children) {
      this.getState(n).parent = node;
      this.updateNode(n);
    }
  }

  getJointMatrices(node, skin) {
    const jointMatrices       = new Float32Array(skin.joints.length * 16);
    const jointNormalMatrices = new Float32Array(skin.joints.length * 16);
