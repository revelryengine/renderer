...
export class Texture extends NamedGLTFProperty {
   ...
   */
  dereference() {
    super.dereference('sampler', 'samplers');
    super.dereference('source', 'images');
  }
   ...
   */
  rereference() {
    super.rereference('sampler', 'samplers');
    super.rereference('source', 'images');
  }
  ...
  createTexture(context) {
    ...
    context.texImage2D(GL.TEXTURE_2D, 0, GL.RGBA, GL.RGBA, GL.UNSIGNED_BYTE, $data);

    if (MIMAP_FILTERS.has(minFilter)) {
