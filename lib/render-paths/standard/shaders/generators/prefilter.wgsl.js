...
import cubecoord      from '../../../common/shaders/generators/cubecoord.wgsl.js';

import generateFormulasBlock from '../../../common/shaders/generators/formulas.wgsl.js';

import { Prefilter } from '../prefilter-shader.js';
...
export function generate({ input: { sampleCount, distribution = 0, lodBias = 0 } }) {
    const vertex = fullscreenVert();

    const fragment = /* wgsl */`

        @group(0) @binding(0) var colorSampler: sampler;
        @group(0) @binding(1) var colorTexture: texture_cube<f32>;

        ${Prefilter.generateUniformBlock('wgsl', 1, 0)}
        ...
        fn computeLod(pdf: f32) -> f32{
            ...
            // https://cgg.mff.cuni.cz/~jaroslav/papers/2007-sketch-fis/Final_sap_0073.pdf
            return 0.5 * log2( 6.0 * size * size / (f32(${sampleCount}) * pdf));
        }
        ...
        fn filterColor(N: vec3<f32>) -> vec3<f32> {
            ...
            var roughness = prefilter.roughness;
            var lodBias   = f32(${lodBias});

            for(var i = 0; i < ${sampleCount}; i = i + 1) {
                var importanceSample = getImportanceSample(i, N, roughness, ${distribution}, ${sampleCount});
    ...
    const fragment = /* wgsl */`
        ...
        fn LUT(NdotV: f32, roughness: f32) -> vec3<f32>{
            ...
            for(var i = 0; i < ${sampleCount}; i = i + 1) {
                ...
                var importanceSample = getImportanceSample(i, N, roughness, ${GGX}, ${sampleCount});
            ...
            return vec3<f32>(4.0 * A, 4.0 * B, 4.0 * 2.0 * ${M_PI} * C) / f32(${sampleCount});
        }
