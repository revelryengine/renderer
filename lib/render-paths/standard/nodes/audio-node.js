...
export class AudioNode extends RenderNode {
    ...
    run(commandEncoder, { graph, frustum }) {
        ...
        this.audio.volume = this.settings.audio.volume;
        this.audio.muted  = this.settings.audio.muted;
    ...
    createNodeEmitter(graph, node) {
        ...
        const emitterPan = new PannerNode(this.audio.context, {
            panningModel: 'HRTF',
            ...emitter.positional,
            coneInnerAngle: rad2Deg(emitter.positional.coneInnerAngle),
            coneOuterAngle: rad2Deg(emitter.positional.coneOuterAngle),
        });
