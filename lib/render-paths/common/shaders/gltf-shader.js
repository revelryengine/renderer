...
import { normalizers, WeakCache } from '../../../../deps/utils.js';
...
export class GLTFShader extends Shader {
    ...
    getFlags() {
        ...
        const flags = {
            ...
            useTransmission   : settings.enabled.transmission && material.extensions?.KHR_materials_transmission,
            useEnvironment    : settings.enabled.environment,
            usePunctual       : settings.enabled.punctual,
            useShadows        : settings.enabled.punctual && settings.enabled.shadows,
            useSSAO           : settings.enabled.ssao && !material.extensions?.KHR_materials_transmission,
            useFog            : settings.enabled.fog,
    ...
    #getBufferFromAccessor({ accessor, usage, name, deindex, indices }) {
        const cache = deindex && indices ? GLTFShader.#accessorBufferCache.ensure(indices, this.gal, accessor, () => ({})) : GLTFShader.#accessorBufferCache.ensure(this.gal, accessor, () => ({}));
    ...
    static #getAccessorVertexFormat(accessor) {
        ...
        let format = `${VERTEX_FORMAT.gltf[accessor.normalized ? 'normalized' : 'unnormalized'][accessor.componentType]}${number > 1 ? `x${number}` : ''}`;
    ...
    #getMorphTargetTexture({ primitive, deindex }) {
        const cache = deindex && primitive.indices ? GLTFShader.#morphTargetCache.ensure(primitive.indices, primitive, () => ({})) : GLTFShader.#morphTargetCache.ensure(primitive, () => ({}));
    ...
    #createMorphTargetTexture({ primitive, deindex }) {
        ...
        const layersPerTarget = Object.values(hasTarget).length;
        ...
        const texture = this.gal.device.createTexture({ format: 'rgba32float', size: { ...size, depthOrArrayLayers: targetCount * layersPerTarget }, usage, array: true });
