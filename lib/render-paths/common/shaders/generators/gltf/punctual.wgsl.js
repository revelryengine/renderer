    ...
    const code = /* wgsl */`
        ...
        fn getSpotAttenuation(pointToLight: vec3<f32>, spotDirection: vec3<f32>, outerConeCos: f32, innerConeCos: f32) -> f32 {
            ...
            if (actualCos > outerConeCos) {
                if (actualCos < innerConeCos) {
                    var angularAttenuation = (actualCos - outerConeCos) / (innerConeCos - outerConeCos);
        ...
        ${flags.useShadows ? /* wgsl */`
            ...
            fn sampleDepth(layer: i32, uv: vec2<f32>, depth: f32) -> f32 {
                ...
                return textureSampleCompareLevel(shadowsTexture, shadowsSampler, uv, layer, saturate(depth));
            }
            ...
            fn shadowPCF(layer: i32, bias: f32)  -> f32 {
                ...
                var position = texcoord.xyz * (1.0 / texcoord.w);
        ...
        fn applyPunctualLight(light: Light, v: vec3<f32>, n: vec3<f32>, NdotV: f32) {
            ...
            ${flags.useTransmission ? /* wgsl */`
                ...
                var transmissionRay = getVolumeTransmissionRay(n, v, materialInfo.thickness, materialInfo.ior, normalInfo.scale);
        ...
        fn applyPunctual(){
            ...
            // Unrolling is more performant in WebGPU here

            // var count = min(punctual.lightCount, ${punctual.maxLights});
            // for(var i = 0; i < count; i += 1) {
            //     applyPunctualLight(punctual.lights[i], v, n, NdotV);
            // }

            ${unroll(/* wgsl */`applyPunctualLight(punctual.lights[$$i], v, n, NdotV); if($$i > punctual.lightCount) { return; }`, punctual.maxLights)}
        }
    `
