...
export function generate(shader) {
    ...
    const vertex = /* glsl */`#version 300 es
        ...
        void main(void){
            ...
            v_modelInfo0 = vec4(normalize((modelInfo.normalMatrix * vec4(getNormal(modelInfo), 0.0)).xyz), 0.0);
            ${flags.hasAttr['TANGENT'] ? /* glsl */`
            v_modelInfo1 = vec4(normalize((modelInfo.matrix * vec4(getTangent(modelInfo), 0.0)).xyz), 0.0);
            v_modelInfo2 = vec4(cross(v_modelInfo0.xyz, v_modelInfo1.xyz) * a_tangent.w, 0.0);
            ` : ''}
            ` : ''}
