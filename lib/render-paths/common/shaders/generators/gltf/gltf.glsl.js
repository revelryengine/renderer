...
export function generate({ flags, locations, input: { renderNode, settings } }) {
    const color0Type   = flags.hasColor0Vec4   ? 'vec4': 'vec3';

    const vertex = generateVertexBlock({ flags, locations, punctual: renderNode.punctual });
    ...
    const fragment = /* glsl */`#version 300 es
        ...
        ${flags.lighting === 'standard' || flags.lighting === 'preview' ? generateLightingBlock({ flags, locations, punctual: renderNode.punctual }) : ''}
