...
export function generate({ flags, locations, input: { settings } }) {  
    ...
    const lightingMode = {
        ...
        `,
        solid: /* wgsl */`
            out.color = vec4<f32>(linearTosRGB(material.baseColorFactor.rgb * dot(normalInfo.n, normalInfo.v)), 1.0);
        `,
        preview: /* wgsl */`
            out.color = vec4<f32>(linearTosRGB(materialInfo.baseColor.rgb * dot(normalInfo.n, normalInfo.v)), materialInfo.baseColor.a);
        `,
        wireframe: /* wgsl */`
