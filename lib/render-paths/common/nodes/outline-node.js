...
class OutlineExtractNode extends RenderNode {
    ...
    reconfigure({ id }) {
        ...
        const view   = id.texture.createView();
        const format = id.texture.format;

        this.outlineShader = new OutlineShader(this.gal, { label: 'OutlineNode', view, format }).compile();
    }
...
export class OutlineNode extends RenderNode {
    ...
    reconfigure({ color }) {
        ...
        this.extractNode.reconfigure({ id: this.gltfNode.output.id });
