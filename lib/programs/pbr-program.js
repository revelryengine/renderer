...
void main()
{
  ...
  #ifdef HAS_SKIN
    mat4 skin =
        a_Weights0.x * u_JointMatrix[int(a_Joints0.x)] +
        a_Weights0.y * u_JointMatrix[int(a_Joints0.y)] +
        a_Weights0.z * u_JointMatrix[int(a_Joints0.z)] +
        a_Weights0.w * u_JointMatrix[int(a_Joints0.w)];

    gl_Position = u_MVPMatrix * skin * morph; // needs w for proper perspective correction
  #else
...
export class PBRProgram extends Program {
   ...
   * @param {Renderer} params.renderer - The rednerer to create the shader with.
  ...
  constructor(context, {
    ...
    brdfTexture, specularTexture, diffuseTexture,
    attributes, targets, extensions = {},
  }) {
    const { NORMAL, TANGENT, TEXCOORD_0, COLOR_0, JOINTS_0 } = attributes;
    const define = {
      USE_IBL: brdfTexture && specularTexture && diffuseTexture ? 1 : 0,
      USE_TEX_LOD: extensions.EXT_shader_texture_lod ? 1 : 0,
  ...
  run(
    ...
    morphWeights,
    jointMatrix,
    { brdfTexture, diffuseTexture, specularTexture } = {},
  ) {
    this.use();
    ...
    this.uniforms.set('u_MVPMatrix', mvpMatrix);
    this.uniforms.set('u_Camera', cameraPos);
    this.uniforms.set('u_MorphWeights[0]', morphWeights);
    this.uniforms.set('u_JointMatrix[0]', jointMatrix);

    const { indices, attributes, mode, material = {}, targets = [] } = primitive;

    for (let t = 0; t < targets.length; t++) {
      if (targets[t].POSITION) this.attributes.set(`a_MorphPosition${t}`, targets[t].POSITION);
      if (targets[t].NORMAL) this.attributes.set(`a_MorphNormal${t}`, targets[t].NORMAL);
      if (targets[t].TANGENT) this.attributes.set(`a_MorphTangent${t}`, targets[t].TANGENT);
    }

    const { POSITION, NORMAL, TANGENT, TEXCOORD_0, COLOR_0, JOINTS_0, WEIGHTS_0 } = attributes;
    ...
    this.uniforms.set('u_MetallicRoughnessValues', [metallicFactor, roughnessFactor]);

    this.attributes.set('a_Position', POSITION);

    if (this.define['HAS_NORMALS']) {
      this.attributes.set('a_Normal', NORMAL);

      if (this.define['HAS_TANGENTS']) {
        this.attributes.set('a_Tangent', TANGENT);
    ...
    if (this.define['HAS_UV']) {
      this.attributes.set('a_UV', TEXCOORD_0);
    }

    if (this.define['HAS_VERTEXCOLORS']) {
      this.attributes.set('a_VertexColor', COLOR_0);
    }

    if (this.define['HAS_SKIN']) {
      this.attributes.set('a_Joints0', JOINTS_0);
      this.attributes.set('a_Weights0', WEIGHTS_0);
    }

    if (this.define['HAS_NORMALMAP']) {
      this.uniforms.set('u_NormalSampler', normalTexture.getWebGLTexture(context));
      this.uniforms.set('u_NormalScale', normalTexture.scale);
    }
    if (this.define['HAS_OCCLUSIONMAP']) {
      this.uniforms.set('u_OcclusionSampler', occlusionTexture.getWebGLTexture(context));
      this.uniforms.set('u_OcclusionStrength', occlusionTexture.strength);
    }
    if (this.define['HAS_EMISSIVEMAP']) {
      this.uniforms.set('u_EmissiveSampler', emissiveTexture.getWebGLTexture(context));
      this.uniforms.set('u_EmissiveFactor', emissiveFactor);
    }
    if (this.define['HAS_BASECOLORMAP']) {
      this.uniforms.set('u_BaseColorSampler', baseColorTexture.getWebGLTexture(context));
    }
    if (this.define['HAS_METALROUGHNESSMAP']) {
      this.uniforms.set('u_MetallicRoughnessSampler', metallicRoughnessTexture.getWebGLTexture(context));
    }

    if (brdfTexture) {
      this.uniforms.set('u_brdfLUT', brdfTexture.getWebGLTexture(context));
    }

    if (diffuseTexture) {
      this.uniforms.set('u_DiffuseEnvSampler', diffuseTexture.getWebGLTexture(context));
    ...
    if (specularTexture) {
      this.uniforms.set('u_SpecularEnvSampler', specularTexture.getWebGLTexture(context));
    }

    // temporary lighting
    this.uniforms.set('u_LightColor', vec3.fromValues(1, 1, 1));
    this.uniforms.set('u_LightDirection', vec3.fromValues(0.7399, 0.6428, 0.1983));
    ...
    if (indices) {
      const { bufferView, count, componentType, byteOffset } = indices;
      context.bindBuffer(context.ELEMENT_ARRAY_BUFFER, bufferView.getWebGLBuffer(context));
