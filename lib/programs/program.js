...
function createAttribSetter(context, location) {
  return (accessor) => {
    const {
      bufferView: { byteStride = 0, target = context.ARRAY_BUFFER },
      componentType = context.FLOAT, normalized = false, numberOfComponents, byteOffset = 0,
    } = accessor;
    const buffer = accessor.createWebGLBuffer(context);
