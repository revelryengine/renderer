...
export const SAMPLER_PARAMS = {
    ...
    minFilterMode: /** @type {const} */({
        ...
        [GL.LINEAR]                 : { filter: 'linear',  mipmap: undefined },
        [GL.NEAREST]                : { filter: 'nearest', mipmap: undefined },
...
export const PBR_DEBUG_MODES = /** @type {const} */({
    ...
    'MaterialInfo Volume Thickness': {
        wgsl: /* wgsl */`out.color = vec4<f32>(linearTosRGB(vec3<f32>(materialInfo.thickness / 10.0)), 1.0);`,
        glsl: /* glsl */`g_finalColor = vec4(linearTosRGB(vec3(materialInfo.thickness / 10.0)), 1.0);`
    },
    'MaterialInfo Transmission': {
        wgsl: /* wgsl */`out.color = vec4<f32>(linearTosRGB(vec3<f32>(materialInfo.transmissionFactor)), 1.0);`,
glsl: /* glsl */`g_finalColor = vec4(linearTosRGB(vec3(materialInfo.transmissionFactor)).rgb, 1.0);`
    },
