...
export class WebGLTF extends GLTFProperty {
  ...
  constructor(glTF = { asset: { version: '2.0', generator: 'WebGLTF Runtime Generation' } }, uri = window.location.href) {
    ...
    // -------- Order of initialization is important from here on -----------------------
  ...
  toJSON() {
    return {
      ...this,
      scene: this.scenes.indexOf(this.scene),
    };
  }
  ...
  async fetch() {
    return Promise.all([
      ...this.images.map(image => image.fetch()),
      ...this.buffers.map(buffer => buffer.fetch()),
    ]);
  }


  /**
   * Initialize all objects within the specified redner target context.
   *
   * @param {Renderer} renderer - The render target to initialize objects with.
   */
  init(target) {
   ...
   */
  createScene(name) {
    const scene = { name, nodes: [] };
    this.scenes.push(scene);
    return scene;
  }

  /**
   * Creates a new camera and adds it to the glTF object
   * @param {String} [type='perspective'] - The type of camera to create. The available types are 'perspective' and 'orthographic'
   * @param {perspective|orthographic} [props={ yfov: 75 * (Math.PI / 180), znear: 0.01 }] - The properties of the scene.
   * @returns {node} - Returns a node with an attached camera.
   */
  createCamera(type = 'perspective', props = { yfov: 75 * (Math.PI / 180), znear: 0.01 }) {
    const camera = { type, [type]: props };
