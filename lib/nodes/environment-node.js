...
const PREFILTER_SAMPLE_SIZE = 1024;
 ...
 */
 class DownsampleNode extends CubeRenderNode {
    ...
    constructor(renderPath, cubemap) {
        ...
        this.downsampleShader = new ResampleShader(this.gal, { 
...
export class EnvironmentNode extends CubeRenderNode {
    ...
    reconfigure() {
        ...
        this.output.envSampler  = this.gal.device.createSampler({ minFilter: 'linear', magFilter: 'linear', mipmapFilter: 'linear' });
    }

    run(commandEncoder, { graph }) {
        this.lutNode.run(commandEncoder);
        if(graph.environment && this.renderPath.settings.environment?.enabled) {
        ...
        this.output.environment.exposure = 1;
    ...
    async #deriveIrradianceCoefficients(commandEncoder, cubemap) {
        ...
        console.debug(`Derived Irradiance Coefficients in ${performance.now() - start}ms`, coefficients);
        return coefficients;
    }
    ...
    #prefilterCubemap(commandEncoder, cubemap) {
        ...
        this.gal.generateMipmap(commandEncoder, source, { format, mipLevelCount, size: { depthOrArrayLayers: 6 }, viewDimension: 'cube' });
