...
class LUTNode extends RenderNode {
    ...
    async runWhenReady() {
        this.shader = new LUTShader(this.gal, { format: this.attachments.colors[0].format, sampleCount: PREFILTER_SAMPLE_SIZE });
...
class ResampleNode extends CubeRenderNode {
    ...
    reconfigure() {
        super.reconfigure();
        this.mipmapGenerator = this.gal.createMipmapGenerator(this.output.color.texture, { viewDimension: 'cube' });
    }
...
class PrefilterNode extends RenderNode {
    constructor(renderer, { source, destination, distribution, mipLevelCount }) {
        ...
        this.initialized   = this.#prepareShaders();
    }
    ...
    render(renderPassEncoder, { face, level }) {    
        this.#prefilterShaders[level].run(renderPassEncoder, face);
    }
    ...
    async #prepareShaders() {
        for(let level = 0; level < this.mipLevelCount; level++) {
            ...
            this.#prefilterShaders[level] = new PrefilterShader(this.gal, { view, format: 'rgba16float', distribution, roughness, sampleCount: PREFILTER_SAMPLE_SIZE });
        }

        return Promise.all(this.#prefilterShaders.map(shader => shader.initialized));
    }
}

class PrefilterEnvironmentNode extends RenderNode {
    constructor(renderer, { source, textures }) {
        super(renderer);
...
export class EnvironmentNode extends CubeRenderNode {
    ...
    reconfigure() {
        ...
        this.output.environment = new Environment(this.renderer.gal, { mipLevelCount: ENVIRONMENT_MIPLEVEL_COUNT });
    ...
    #createPrefilterNode(cubemap) {
        ...
        return new PrefilterEnvironmentNode(this.renderer, { source, textures: { ggx: envGGX.texture, charlie: envCharlie.texture } });
    ...
    async #prefilterCubemap(cubemap) {
        const commandEncoder = this.gal.device.createCommandEncoder();
        ...
        await Promise.all([ggx.initialized, charlie.initialized]);
