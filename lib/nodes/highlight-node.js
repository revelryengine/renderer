...
import { SolidProgram     } from '../programs/gltf/solid-program.js';
import { WireframeProgram } from '../programs/gltf/wireframe-program.js';
            ...
            for(const node of highlightNode.depthFirstSearch()) {
                ...
                if(mesh) {
                    for (const primitive of mesh.primitives) {
                        ...
                        program.run({ graph, primitive, node, frustum, input: { color: fill } });
                    }
                }
            }
            ...
            for(const node of highlightNode.depthFirstSearch()) {
                ...
                if(mesh) {
                    for (const primitive of mesh.primitives) {
                        ...
                        program.run({ graph, primitive, node, frustum, input: { color: stroke } });
                    }
                }
            }
        }
