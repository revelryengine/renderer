...
export class GLTFNode extends RenderNode {
    ...
    attachments = {
        colors: {
            color:  { location: 0,                       },
            accum:  { location: 1, format: 'rgba16float', clearValue: [0, 0, 0, 1] },
            reveal: { location: 2, format: 'r16float'    },
    ...
    reconfigure() {
        ...
        if(alpha && (color && (color.enabled ?? true))) {
            this.#alphaComposite = {
    ...
    run(commandEncoder, ...args){
        ...
        this.#alphaComposite && this.#runAlphaComposite(commandEncoder, ...args);
    }

    #runAlphaComposite(commandEncoder) {
