...
export class MainNode extends GeometryNode {
    ...
    render(renderPassEncoder, { graph, frustum, instances }) {
        ...
        for(const { primitive, mesh, skin, offset, count } of this.iterateBlocks(blocks, { opaque: true, transmissive: true, alpha: true })) {
            const shader = this.getShader({ primitive, mesh, skin });
            shader.run(renderPassEncoder, { buffer, offset, count });
        }
    }

    #shaders = new WeakMap();
    getShader({ primitive, mesh }) {
        const key = GLTFShader.getShaderKey(primitive);
        return this.#shaders.get(key) || this.#shaders.set(key, new GLTFShader(this.gal, { 
            primitive, mesh, settings: this.renderPath.settings,
    ...
    reconfigure() {
        ...
        const { settings: { debug, environment, punctual, tonemap, fog: { enabled: fogEnabled } } } = this.renderPath;
        const currentSettings = JSON.stringify({ ...debug, environment, punctual, tonemap, fogEnabled });
        ...
        this.bindGroupLayout = this.gal.device.createBindGroupLayout({
            entries: [
                ...
                //transmission
                { binding: 6, visibility: SHADER_STAGE.FRAGMENT, texture: { viewDimension: '2d' } },
            ],
        });
        ...
        this.bindGroup = this.gal.device.createBindGroup({
            ...
            entries: [
                ...
                //transmission
                { binding: 6, resource: this.getConnectionValue('transmission').texture.createView({ dimension: '2d' }) },
            ]
        });
    }
