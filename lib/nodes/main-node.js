...
export class MainNode extends GeometryNode {
    ...
    reconfigure() {
        ...
        const currentSettings = JSON.stringify({ ...debug, tonemap, temporal, msaaEnabled, msaaSamples,
            environmentEnabled, punctualEnabled, fogEnabled, ssaoEnabled, shadowsEnabled, transmissionEnabled,
        });
    ...
    run(commandEncoder, { graph, frustum, instances }) {
        super.run(commandEncoder, { graph, frustum, instances });

        graph.mirrorHistory(commandEncoder);
    }
}
