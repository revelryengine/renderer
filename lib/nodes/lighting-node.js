...
import { GLTFShadowShader } from '../shaders/gltf-shader.js';
...
export class LightingNode extends GeometryNode {
    ...
    attachments = {
        // colors: [{ name: 'color', }], //uncomment for debugging purposes
        depth: { name: 'depth' },
    }
    ...
    * iterateBlocks(blocks) {
    ...
    reconfigure(...args){
        ...
        this.bindGroup = this.gal.device.createBindGroup({
            label: 'LightingNode BindGroup',
            layout: this.bindGroupLayout,
            entries: []
        });

        this.output.shadowSampler = this.gal.device.createSampler({ minFilter: 'linear', magFilter: 'linear', compare: 'less' });

        super.reconfigure(...args);
    }

    run(commandEncoder, { graph, frustum }) {
        if(!this.renderPath.settings.punctual.enabled) return;
        ...
        if(!this.renderPath.settings.shadows.enabled) return this.lighting.upload();

        const settings = this.renderPath.settings.shadows;

        const { near, far } = frustum;

        this.lighting.shadowCascadeDepths = LightingNode.calculateCascadeDepths(near, far, settings.lambda, settings.cascades);
        this.lighting.shadowCascadeCount  = settings.cascades;

        const splits = [near, ...this.lighting.shadowCascadeDepths.slice(0, settings.cascades - 1), far];

        let layer = 0;
        for(let i = 0, l = this.lighting.lightCount; i < l; i++) {
            const light = this.lighting.lights[i];
            ...
            switch(light.lightType) {
                case LIGHT_TYPES.directional: {
                    ...
                    for(let i = 0; i < settings.cascades; i++) {
                        ...
                        this.setShadowMatrix(this.lighting.shadowMatrices[layer + i], shadowFrustum);
                    }
                ...
                case LIGHT_TYPES.spot: {
                    ...
                    this.setShadowMatrix(this.lighting.shadowMatrices[layer], shadowFrustum);
                    layer++;
                    break;
                }
            }
        ...
        this.lighting.shadowCount = layer;
    ...
    getShadowFrustum(light, frustum, near, far) {
        ...
        if(light.lightType === LIGHT_TYPES.spot) {
            shadowFrustum = this.#spotShadowFrustums.get(light) || this.#spotShadowFrustums.set(light, new Frustum(this.gal)).get(light);
            ...
            shadowFrustum = shadowFrustums[near] || (shadowFrustums[near] = new Frustum(this.gal));
        ...
        shadowFrustum.update({ transform, projectionMatrix, width: this.size.width, height: this.size.height });
