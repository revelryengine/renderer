...
export class BaseNode extends GeometryNode {
    ...
    renderOpaque(renderPassEncoder, { graph, instances }) {
        super.renderOpaque(renderPassEncoder, { graph, instances });
    ...
    reconfigure() {
        ...
        const currentSettings = JSON.stringify({ ...debug, tonemap, 
        ...
        if(environmentEnabled && skyboxEnabled) {
            this.skyShader = new SkyShader(this.gal, { renderNode: this, renderNode: this });
        } else {
            this.skyShader = null;
        }  
