...
export class PunctualNode extends GeometryNode {
    ...
    getShadowFrustum(light, frustum, near, far) {
        ...
        if(light.lightType === LIGHT_TYPES.spot) {
            shadowFrustum = this.#spotShadowFrustums.get(light) || this.#spotShadowFrustums.set(light, this.renderPath.createFrustum({ x: 0, y: 0, width: this.size.width, height: this.size.height })).get(light);
            ...
            shadowFrustum = shadowFrustums[near] || (shadowFrustums[near] = this.renderPath.createFrustum({ x: 0, y: 0, width: this.size.width, height: this.size.height }));
