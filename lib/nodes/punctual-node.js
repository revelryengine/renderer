...
/**
 * A shadow frustum has a simpler update as it does not need a graph or cameraNode
 */
class ShadowFrustum extends Frustum {
    update({ transform, projectionMatrix }) {
 ...
 */
export class PunctualNode extends GeometryNode {
    ...
    reconfigure(...args){
        this.punctual        = new Punctual(this.renderer.gal);
    ...
    run(commandEncoder, { graph, frustum }) {
        ...
        if(!this.renderer.settings.shadows.enabled) return this.punctual.upload();

        const settings = this.renderer.settings.shadows;
    ...
    getShadowFrustum(light, frustum, near, far) {
        ...
        if(light.lightType === LIGHT_TYPES.spot) {
            shadowFrustum = this.#spotShadowFrustums.get(light) || this.#spotShadowFrustums.set(light, new ShadowFrustum(this.gal, { x: 0, y: 0, width: this.size.width, height: this.size.height })).get(light);
            ...
            shadowFrustum = shadowFrustums[near] || (shadowFrustums[near] = new ShadowFrustum(this.gal, { x: 0, y: 0, width: this.size.width, height: this.size.height }));
