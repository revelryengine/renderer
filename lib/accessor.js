...
export class Accessor extends NamedGLTFProperty {
   ...
   */
  createTypedView() {
    const { bufferView, byteOffset, count, numberOfComponents, componentType } = this;
    const { buffer } = bufferView;
    return new TYPEDARRAYS[componentType](buffer.$data, byteOffset + bufferView.byteOffset, count * numberOfComponents);
  }
}
