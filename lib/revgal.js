...
export class RevGAL {
    ...
    createTextureWithData({ data, ...descriptor }) {
        ...
        if (data instanceof ImageBitmap || data instanceof HTMLCanvasElement || data instanceof OffscreenCanvas) {
    ...
    getTextureFromGLTF(gltfTextureInfo) {
        return this.#gltfTextures.get(gltfTextureInfo) || this.#gltfTextures.set(gltfTextureInfo, this.createTextureFromGLTF(gltfTextureInfo)).get(gltfTextureInfo);
    }

    /**
     * @see https://www.khronos.org/registry/glTF/specs/2.0/glTF-2.0.html#_filtering
     */
    createTextureFromGLTF(gltfTextureInfo){
        if(gltfTextureInfo.extensions?.KHR_texture_basis) return this.#createTextureFromGLTFBasisU(gltfTextureInfo);
        ...
        if(extensions?.KHR_texture_basisu) {
            const texture = this.#createTextureFromGLTFBasisU(extensions.KHR_texture_basisu, sRGB);
    ...
    #createTextureFromGLTFBasisU(basisuInfo, sRGB) {
        ...
        const astcSupported = this.device.features.has('texture-compression-astc');
        const bc7Supported  = this.device.features.has('texture-compression-bc');
        const etcSupported  = this.device.features.has('texture-compression-etc');
        ...
        basisuInfo.transcode({ astcSupported, bc7Supported, etcSupported }).then(data => {
    ...
    static getAccessorVertexFormat(accessor) {
        ...
        return `${VERTEX_FORMAT.gltf[accessor.normalized][accessor.componentType]}${number > 1 ? `x${number}` : ''}`;
    }
    ...
    getMaterialUBO(material = this.#defaultMaterial){
        return this.#materialUBOs.get(material) || this.#materialUBOs.set(material, new Material(this, material)).get(material);
    }
