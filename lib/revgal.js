...
export class RevGAL {
    ...
    #vertShaders = new Map();
    #fragShaders = new Map();
    generateShaders(source, input) {
        if(typeof source === 'function') {
            source = source(input);
        }
        const { cacheKey } = input;
        const { vertex, fragment, flags } = source;
        const vertShader = this.#vertShaders.get(cacheKey) || this.#vertShaders.set(cacheKey, this.device.createShaderModule({ code: vertex,   glType: GL.VERTEX_SHADER })).get(cacheKey);
        const fragShader = this.#fragShaders.get(cacheKey) || this.#fragShaders.set(cacheKey, this.device.createShaderModule({ code: fragment, glType: GL.FRAGMENT_SHADER })).get(cacheKey);
