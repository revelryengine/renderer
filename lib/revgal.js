...
export class RevGAL {
    ...
    generateShaders(shaderConstructor, input) {
        ...
        const { vertex, fragment, flags } = source;
        const vertShader = this.#vertShaders.get(cacheKey) || this.#vertShaders.set(cacheKey, this.device.createShaderModule({ label: `VertShader:${name}`, code: vertex,   glType: GL.VERTEX_SHADER })).get(cacheKey);
        const fragShader = this.#fragShaders.get(cacheKey) || this.#fragShaders.set(cacheKey, this.device.createShaderModule({ label: `FragShader:${name}`, code: fragment, glType: GL.FRAGMENT_SHADER })).get(cacheKey);
